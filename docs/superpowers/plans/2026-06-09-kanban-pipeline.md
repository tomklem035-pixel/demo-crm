# Kanban Pipeline Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a `/pipeline` kanban board with drag-to-move deal cards across 4 active stages, click-to-edit, and a New Deal modal.

**Architecture:** A server component (`page.tsx`) fetches all active-stage deals with company and contact data, serialises Decimal values, and passes them to a client component (`PipelineView.tsx`). The client builds a `Record<ActiveStage, Deal[]>` board state, renders 4 columns with deal cards, and uses `@dnd-kit/core` for drag-and-drop. Stage changes are sent via `PATCH /api/deals/[id]` with optimistic updates. Clicking a card opens an edit modal; a "+ New Deal" button creates new deals via `POST /api/deals`.

**Tech Stack:** Next.js 14 App Router, Prisma, TypeScript, Tailwind CSS, @dnd-kit/core

---

### Task 1: Install @dnd-kit and add Pipeline nav link

**Files:**
- Modify: `package.json` (via npm install)
- Modify: `src/components/Sidebar.tsx`

- [ ] **Step 1: Install @dnd-kit/core and @dnd-kit/utilities**
...
- [ ] **Step 2: Add Pipeline link to Sidebar**
...
```bash
git add package.json package-lock.json src/components/Sidebar.tsx
git commit -m "feat: install @dnd-kit and add Pipeline nav link"
```

---

### Task 2: Create the server component and stub

**Files:**
- Create: `src/app/pipeline/page.tsx`
- Create: `src/app/pipeline/PipelineView.tsx` (stub — replaced in Task 3)
...
- [ ] **Step 2: Create stub `src/app/pipeline/PipelineView.tsx`**
...
export default function PipelineView({
  ...
  return <div className="p-4 text-slate-500">Pipeline — coming soon</div>;
}
```
...
```bash
git add src/app/pipeline/page.tsx src/app/pipeline/PipelineView.tsx
git commit -m "feat: add pipeline server component"
```

---

### Task 3: Build PipelineView — static board with modals

**Files:**
- Modify: `src/app/pipeline/PipelineView.tsx` (replace stub entirely)

This task builds the complete working board: 4 column layout, deal cards, edit modal, new deal modal. Cards are click-to-edit only — drag-and-drop is added in Task 4.

- [ ] **Step 1: Replace the stub with the full static PipelineView**

Overwrite `src/app/pipeline/PipelineView.tsx` with:
...
```bash
git add src/app/pipeline/PipelineView.tsx
git commit -m "feat: add pipeline board layout with modals"
```

---

### Task 4: Add drag-and-drop

**Files:**
- Modify: `src/app/pipeline/PipelineView.tsx` (complete overwrite adding DnD)

This task rewrites `PipelineView.tsx` to add `@dnd-kit/core` drag-and-drop. The board logic, modals, and types are identical to Task 3 — only the drag infrastructure is added: `DraggableCard` and `DroppableColumn` local components, `DndContext` wrapping, `handleDragEnd` with optimistic update + API call + revert on failure.

- [ ] **Step 1: Overwrite `src/app/pipeline/PipelineView.tsx` with the drag-enabled version**
...
```bash
git add src/app/pipeline/PipelineView.tsx
git commit -m "feat: add drag-and-drop to pipeline board"
```
...
- [ ] "Pipeline" appears in the sidebar between Deals and Analytics
- [ ] `/pipeline` renders with 4 columns: Prospecting, Qualification, Proposal, Negotiation
- [ ] Active-stage deals appear in the correct column; CLOSED_WON / CLOSED_LOST deals are absent
- [ ] Column headers show the stage name, deal count badge, and column value total
- [ ] Deal cards show title, contact/company avatar + name, value, and expected close date
- [ ] Empty columns show "No deals" placeholder
- [ ] Clicking a deal card opens the edit modal pre-filled with the deal's data
- [ ] Saving the edit modal with an active stage moves the card to the correct column instantly
- [ ] Saving the edit modal with CLOSED_WON or CLOSED_LOST removes the card from the board
- [ ] "+ New Deal" opens the creation modal; saving adds the card to the correct column
- [ ] Dragging a card to a different column moves it immediately; stage updates in the DB
- [ ] Dragging a card onto its own column does nothing
- [ ] If a drag PATCH fails (simulate with network offline), the card reverts and shows a 3-second error
